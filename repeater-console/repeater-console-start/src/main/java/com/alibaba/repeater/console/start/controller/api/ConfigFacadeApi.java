...
public class ConfigFacadeApi {
    ...
    public RepeaterResult<RepeaterConfig> getConfig(@PathVariable("appName") String appName,
        ...
        // 白名单列表
        config.setHttpEntrancePatterns(Lists.newArrayList("^/dataGrid2/.*$"));
        // java入口方法
        behaviors.add(new Behavior("cn.cmvideo.datagrid2.ng.controller.SearchController", "chargeSearch"));
        config.setJavaEntranceBehaviors(behaviors);
        List<Behavior> subBehaviors = Lists.newArrayList();
        // java调用插件
        subBehaviors.add(new Behavior("cn.cmvideo.datagrid2.ng.remote.IAuthRemote", "queryDeliveryHistory"));
